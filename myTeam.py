...
class MainAgent(CaptureAgent):
  ...
  def chooseAction(self, gameState):
    ...
    if self.atCenter == False and ((self.index == 1) or (self.index == 0))  :
      evaluateType = 'start'

    # If at centre, switch to attack
    if agentPos == self.center and self.atCenter == False and ((self.index == 1) or (self.index == 0)):
      self.atCenter = True
      evaluateType = 'attack'
  ...
  def getFeaturesAttack(self, gameState, action):
    ...
    if(len(foodList)>0):
      features['pickupFood'] = -len(foodList) +self.getScore(successor)
      print  features['pickupFood'] 
    ...
    if self.inEnemyTerritory(gameState) and ((self.index == 1) or (self.index == 0))  :
       x,y = zip(myPos)
       xl = x[0] -1
       xr = x[0] +1
       yu = y[0] +1
    ...
    if (myPosLeft in walls and myPosDown in walls and myPosRight in walls):
       features['threeWalls'] = -1

    if (myPosLeft in walls and myPosUp in walls and myPosRight in walls):
       features['threeWalls'] = -1

    if (myPosLeft in walls and myPosUp in walls and myPosDown in walls):
       features['threeWalls'] = -1

    if (myPosRight in walls and myPosUp in walls and myPosDown in walls):
       features['threeWalls'] = -1
    ...
    if gameState.getAgentState(self.index).scaredTimer == 0:   
        ...
        if myPos!= None:
          ...
          if (newx, newy) in neighbors:
            features["closeInvader"] += -10000
            features["eatInvader"] = -10000
          elif (newx, newy) == ghostpos:
            features["eatInvader"] = -10000
 

    #for agent in enemies:
     #print gameState.getAgentState(agent)
  ...
  def getWeightsAttack(self, gameState, action): 
    return {'successorScore': 800, 'distanceToFood': -10, 'danger': -1000,
            ...
            'distToAlly': -6000, 'pickupCapsule': 5000,'threeWalls':300}
  ...
  def getWeightsDefend(self, gameState, action):
    return {'numInvaders': -10000, 'invaderDistance': -500, 'stop': -5000,
            'reverse': -200, 'danger': 3000, 'distanceToAlly': -4000,'eatInvader':10000,'closeInvader':0}
